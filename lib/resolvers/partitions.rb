    ...
    class Partitions < BaseResolver
      ...
      class << self
        ...
        def read_partitions(fact_name)
          ...
          blocks = Dir.entries(BLOCK_PATH).reject { |dir| dir =~ /^\.+/ }
          blocks.each do |block|
            block_path = "#{BLOCK_PATH}/#{block}"
            if File.directory?("#{block_path}/device")
              device_for_path_exists(block_path)
            elsif File.directory?("#{block_path}/dm")
              dm_for_path_exists(block_path)
            elsif File.directory?("#{block_path}/loop")
              loop_for_path_exists(block_path)
            end
          end
          @fact_list[fact_name]
        end

        def device_for_path_exists(block_path)
        ...
        def dm_for_path_exists(block_path)
        ...
        def loop_for_path_exists(block_path)
