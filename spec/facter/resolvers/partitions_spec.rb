...
describe Facter::Resolvers::Partitions do
  ...
  context 'when /sys/block is not readable' do
    ...
    it 'returns empty hash' do
      result = Facter::Resolvers::Partitions.resolve(:partitions)
  ...
  context 'when /sys/block is readable' do
    ...
    context 'when block have a device subdir' do
      ...
      before do
        allow(File).to receive(:readable?).with(sys_block_path).and_return(true)
        allow(Dir).to receive(:entries).with(sys_block_path).and_return(['.', '..', 'sda'])
      ...
      it 'return partitions fact' do
        ...
        expect(result).to eq(parts)
      end
    end

    context 'when block have a dm subdir' do
      let(:sys_block_subdirs) { ['.', '..', 'sda'] }
    ...
    context 'when block have a loop subdir' do
      let(:sys_block_subdirs) { ['.', '..', 'sda'] }
